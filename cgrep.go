...
func main() {
	...
	var skipFileList skipFileArray
	...
	flag.Var(&patternList, "e", "Pattern [regex]")
	flag.Var(&invertList, "v", "Invert match [regex]")
	flag.Var(&skipDirList, "s", "Skip dir")
	flag.Var(&skipFileList, "skipfile", "Skip file [regex]")
	...
	walkThroughDirs(compiledPatternList, invertList, skipDirList, skipFileList, dirs, *concFlag, encoding, *filenameOnlyFlag)
}
...
func setSkipDirs(sda skipDirArray, defaultSkipDirs []string, allFlag bool) skipDirArray {
...
func walkThroughDirs(
	...
	for i := 0; i < len(dirs); i++ {
		err := filepath.Walk(dirs[i], func(path string, info os.FileInfo, err error) error {
			...
			// In case of file
			if !info.IsDir() {
				if len(skipFileList) > 0 && matchArray(info.Name(), skipFileList) {
					return nil
				}
